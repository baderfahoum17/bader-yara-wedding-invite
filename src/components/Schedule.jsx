...
export default function Schedule() {
  return (
    <Band id="schedule" tone="wine">
      <div className="md:col-span-4">
        <Reveal>
          <Title className="text-ivory">The day</Title>
        </Reveal>
        <Reveal delay={0.1}>
          <p className="mt-6 font-display text-xl text-ivory/70 md:text-2xl">
            {weekday}, {longDate}
          </p>
        </Reveal>
      </div>
        ...
        {schedule.map((item, i) => (
          <li key={item.time} className="relative pt-7">
            ...
            <Reveal delay={0.25 + i * 0.15} y={16}>
              <span className="tabular block font-display text-5xl font-light leading-none text-ivory md:text-6xl">
                {item.time}
              </span>
              <span className="mt-4 block max-w-[22ch] font-display text-xl leading-snug text-ivory/80 md:text-2xl">
                {item.title}
              </span>
            </Reveal>
          </li>
